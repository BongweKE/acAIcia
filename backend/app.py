...
def process_query_async(query_id: str, user_query: str, session_id: str = None, user_id: str = None, conversation_history: list = None):
    ...
    # Fetch user custom instructions if user_id provided
    custom_instructions = ""
    if user_id:
        try:
            ...
            if profile_res.data and profile_res.data[0].get("custom_instructions"):
                ...
                logger.info(f"Loaded custom research instructions for user {user_id}: {custom_instructions[:60]}...")
        except Exception as p_err:
            logger.warning(f"Could not load profile for user {user_id}: {p_err}")

    # -----------------------------------------------------------------------
    # Step 0: Check Semantic Response Cache (<50ms lookup)
    # -----------------------------------------------------------------------
    try:
        ...
        if cache_res.data and len(cache_res.data) > 0:
            ...
            logger.info(f"⚡ Semantic cache HIT for query: '{user_query}' (similarity: {cached_item.get('similarity'):.4f})")
        ...
        logger.warning(f"Semantic cache lookup exception (continuing normal pipeline): {cache_err}")
    ...
    try:
        # -------------------------------------------------------------------
        # 1 & 2. Upgraded Guardian + Architect Agents (Concurrent)
        # -------------------------------------------------------------------
        guardian_prompt = f"""
        ...
        # -------------------------------------------------------------------
        # 3. Retrieval Step (Hybrid RRF Search: Dense Vector + Full-Text)
        # -------------------------------------------------------------------
        r_start = time.time()
        ...
        try:
            ...
            logger.warning(f"Hybrid search RPC failed ({hybrid_err}), falling back to vector match...")
            try:
                ...
                logger.error(f"Vector fallback exception: {rpc_err}")
                results = []
        ...
        # Finalize Telemetry & Log
        telemetry["latency_ms"] = int((time.time() - start_time) * 1000)
        telemetry["total_tokens_used"] = total_tokens

        try:
            supabase.table("query_interaction_logs").insert(telemetry).execute()
            logger.info(f"Successfully logged query telemetry (latency: {telemetry['latency_ms']}ms).")
        except Exception as e:
            logger.error(f"Failed to insert telemetry: {e}")

        # Write to Semantic Cache if database match was found
        if results and synth_text:
            try:
                ...
                logger.info("Saved query response to semantic cache.")
            except Exception as sc_err:
                logger.warning(f"Could not save to semantic cache: {sc_err}")

        # Save chunk logs to query_chunk_logs
...
def fastapi_app_entrypoint():
    ...
    def submit_feedback(req: FeedbackRequest):
        try:
            ...
            logger.info(f"Logged user feedback rating={req.rating} for log_id={req.log_id}")
    ...
    def handle_query(request: QueryRequest):
        ...
        try:
            ...
            logger.info(f"Spawned background query job {query_id}.")
